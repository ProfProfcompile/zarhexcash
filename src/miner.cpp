...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ZARHEXCASHMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ZARHEXCASHMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ZARHEXCASHMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zarhexcash-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("ZARHEXCASHMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running ZARHEXCASHMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadZARHEXCASHMiner() exception");
    } catch (...) {
        LogPrintf("ThreadZARHEXCASHMiner() exception");
    }

    LogPrintf("ThreadZARHEXCASHMiner exiting\n");
}
