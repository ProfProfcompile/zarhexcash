...
// Copyright (c) 2019 The ZARHEXCASH developers
...
#if defined(NDEBUG)
#error "ZARHEXCASH cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "ZARHEXCASH Signed Message:\n";
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount) 

 {
	...
	// 80% for Masternodes from block 1000
	if (nHeight <= 35000) {
    ...
    if (!tx.IsCoinBase()) {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            if (coins->IsCoinBase() || coins->IsCoinStake()) {
                if (nSpendHeight - coins->nHeight < (coins->nHeight < 71053 ? 100 : Params().COINBASE_MATURITY()))
...
void ThreadScriptCheck() {
    RenameThread("zarhexcash-scriptch");
