...
/* *** BEGIN DARKSEND MAZARH - ZARHEXCASH **********
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat ZARHEXCASH and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 ZARHEXCASH per transaction on average.
            */
            if (r <= 10) {
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeZARHEXCASHAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100ZARH+1 ( bit on if present )
    // bit 1 - 10ZARH+1
    // bit 2 - 1ZARH+1
    // bit 3 - .1ZARH+1
    // bit 3 - non-denom
...
void ThreadCheckDarKsendPool()
{
    ...
    RenameThread("zarhexcash-Darksend");
