...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ZARHEXCASH address (e.g. %1)").arg("GVXmy14r8hmuzt1d5ygnqzZfbuomg1XLmF"));
#endif
...
{
    // return if URI is not valid or is no ZARHEXCASH: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::ZARH, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert zarhexcash:// to zarhexcash:
    //
    //    Cannot handle this later, because zarhexcash:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ZARH, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open zarhexcash.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ZARHEXCASH.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for ZARHEXCASH.lnk
...
{
    return GetAutostartDir() / "zarhexcash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a zarhexcash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=ZARHEXCASH\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the zarhexcash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add zarhexcash app to startup item list
